package com.zhoujixing.studyTest;

import  com.zhoujixing.config.BusinessThread;
import com.zhoujixing.entity.VoteEntity;
import com.zhoujixing.web.TestController;
...
public class TestThreadPoolManager implements BeanFactoryAware {
    ...
    /**将任务加入订单线程池*/
    public void addOrders(VoteEntity voteEntity){
        System.out.println("此订单准备添加到线程池，订单号：" );
        //验证当前进入的订单是否已经存在

            cacheMap.put(voteEntity.getX_tpr(), new Object());
            //调入方法
            System.out.println("线程池层:"+voteEntity.getX_tpr());
            BusinessThread businessThread = new BusinessThread(voteEntity);

            System.out.println("thireadpool:"+threadPool.getQueue().size());
